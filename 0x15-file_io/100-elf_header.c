...
/**
 * print_magic - Prints the magic numbers of a header
 * @e_ident: A pointer to an array
...
void print_magic(unsigned char *e_ident)
{
	...
	for (chec = 0; chec < EI_NIDENT; chec++)
	{
		printf("%02x", e_ident[chec]);
 ...
 * @e_type: The ELF type.
 * @e_ident: A pointer to an array that contains the class ELF
 */
void print_type(unsigned int e_type, unsigned char *e_ident)
{
